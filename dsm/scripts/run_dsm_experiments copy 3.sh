...
# --- COPY APPLICATION (replace histogram with variable if needed) ---
...
sudo rm -f /tmp/haltcode

DSM=$THREADS ./${APP} \
    /users/EdoDale/phoenix/phoenix-2.0/tests/input/histogram_datafiles/large.bmp \
    100 &

app_pid=$!
echo "App running as PID $app_pid"
...
sudo ~/criu/criu/criu dump -t "$app_pid" \
